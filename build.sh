...
VO=1 python3 audio_stems.py | tail -1
# 3) mix with the fitted music bed, then two-pass loudnorm to -14 LUFS / -1.5 dBTP
python3 mix_final.py stem_rh out/mix.wav
J=$(ffmpeg -hide_banner -nostats -i out/mix.wav -af loudnorm=I=-14:TP=-1.5:LRA=11:print_format=json -f null - 2>&1 | sed -n '/{/,/}/p')
set -- $(echo "$J" | python3 -c "import json,sys;d=json.load(sys.stdin);print(d['input_i'],d['input_tp'],d['input_lra'],d['input_thresh'],d['target_offset'])")
