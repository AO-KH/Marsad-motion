...
#        SUB=1 ./build_demo.sh <slug>                   a fast draft without motion blur (about 4x quicker)
...
for F in $($PY tools/make_demo.py "$SLUG" --formats); do
  ...
  rm -rf "frames/$SLUG-$F"
  if [ "$SUB" -gt 1 ]; then
    node render_mb.js "$JOBS" "build/$SLUG-$F.html" "frames/$SLUG-$F" "$SUB" && $PY tools/blend.py "frames/$SLUG-$F"
  else
