...
# entrances decelerate (fast start): the visible motion begins on the beat instead of ~0.1 s after it
rep("  emph:p=>{const c=cubicBezier(0.05,0.7,0.1,1);return c(p);},","  emph:p=>{const c=cubicBezier(0.05,0.7,0.1,1);return c(p);},\n  dec:p=>{const c=cubicBezier(0,0,0.2,1);return c(p);},")
rep("function enter(e,t,a,dur,dy=24){const p=ez.prem(P(t,a,a+dur));","function enter(e,t,a,dur,dy=24){const p=ez.dec(P(t,a,a+dur));")
open('film_final5.html','w',encoding='utf-8').write(s); print('film_final5.html',len(s))
