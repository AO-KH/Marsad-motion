...
# absorbs land on the beat grid (8ths, then 16ths through the roll); the last pair is the ignition itself
_MB, _K8 = 0.63832, 9.0197
for i, k in enumerate([10, 10.5, 11, 11.25, 11.5, 11.75]):
    place(absorb(i), _K8 + _MB * (k - 8) - 0.005, pan=(-0.3 if i % 2 else 0.3), gain=FG * 0.46)
place(riser(1.25), _K8 + _MB * 4 - 1.25, gain=FG * 0.36)          # charge-up swell into the downbeat
...
vo = vo * 0.62
np.save('stem_ig_sfx.npy', (sfx * duck_sfx).astype(np.float32))
np.save('stem_ig_vo.npy', vo.astype(np.float32))
np.save('stem_ig_duck.npy', envd.astype(np.float32))
